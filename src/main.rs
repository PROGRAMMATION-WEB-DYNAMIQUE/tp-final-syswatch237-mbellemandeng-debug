...
fn log_event(log_file: &Arc<Mutex<std::fs::File>>, message: &str) {
    if let Ok(mut file) = log_file.lock() {
        let _ = writeln!(file, "{}", message);
    }
}

fn open_log_file() -> Arc<Mutex<std::fs::File>> {
    let file = OpenOptions::new()
        .create(true)
        .append(true)
        .open("syswatch.log")
        .expect("Impossible d'ouvrir syswatch.log");
...
fn handle_client(
    ...
    log_event(&log_file, &format!("[{}] CONNEXION depuis {}", ts, peer_addr));
    ...
    for line in reader.lines() {
        match line {
            Ok(cmd) => {
                ...
                let ts = Local::now().format("%Y-%m-%d %H:%M:%S").to_string();
                log_event(&log_file, &format!("[{}] {} → '{}'", ts, peer_addr, cmd));
    ...
    let ts = Local::now().format("%Y-%m-%d %H:%M:%S").to_string();
    log_event(&log_file, &format!("[{}] DÉCONNEXION de {}", ts, peer_addr));
}
    ...
    for incoming in listener.incoming() {
        match incoming {
            Ok(stream) => {
                ...
                thread::spawn(move || handle_client(stream, shared_snapshot, log_file));
            }
